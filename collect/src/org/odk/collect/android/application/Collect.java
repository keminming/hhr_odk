...
public class Collect extends Application {
    ...
    public static void createODKDirs() throws RuntimeException {
        ...
        for (String dirName : dirs) {
            ...
            if(dirName == FORMS_PATH)
            {
            	AssetManager assetMgr = Collect.getInstance().getAssets();
            	copyAssetFile(assetMgr, "", FORMS_PATH);
            }
        }
    }
    
    private static void copyAssetFile(AssetManager assetMgr, String src, String dst)
    {
    	String[] assetsIWant;
		try {
			
			assetsIWant = assetMgr.list(src);
			
	    	for(String asset : assetsIWant) {    	
	    		if(	assetMgr.list(asset).length > 0)
	    		{
	    			File subdir = new File(FORMS_PATH, asset);
	                if (!subdir.mkdirs()) {
	                    RuntimeException e =
	                            new RuntimeException("ODK reports :: Cannot create directory: "
	                                    + subdir);
		    		...
		    		copyAssetFile(assetMgr,asset,dst + '/' + asset);
	    		}
	    		else
	    		{	    		
		            InputStream in = null;
		            ...
		            try {
		            	if(src != "")
		            		in = assetMgr.open(src + "/" + asset);   // if files resides inside the "Files" directory itself
		            	else
		            		in = assetMgr.open(asset); 
			              out = new FileOutputStream(dst + '/' + asset);
			              copyFile(in, out);
			              in.close();
			              in = null;
			              out.flush();
